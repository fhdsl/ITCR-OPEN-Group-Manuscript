...
\title{Impact of software designed for biomedical research: Are we measuring what we think we are?}
...
\begin{table}
 \caption{\textbf{Software infrastructure enables the capture of valuable metrics for evaluating engagement and impact.} More fine-grained tracking of how users are interacting with web-based tools or websites and can be helpful for later assessments, in addition to improving access, as one can also perform search engine optimization (SEO) to further encourage the visibility. Tools like Altmetric can indicate where  digital object identifiers(DOIs) are cited in multiple sources besides scientific articles, such as blogs, news articles, Wikipedia, social media, and more. }
  \centering
  \begin{tabular} {|p{0.1\textwidth}|p{0.15\textwidth}|p{0.15\textwidth}|p{0.23\textwidth}|p{0.24\textwidth}|}
    \hline
    ...
    \hline
    \multirow{17}{*}{Web Presence}               
    & Readme on Code Repository &  &
    Identify page visits overtime & \\
 \cline{2-5}
    &   \multirow{3}{*}[-3em]{ Web-based tool} & \begin{itemize}
    ...
    \cline{2-5}
    & \multirow{5}{*}{ \shortstack{Documentation \\ Website}} &
    \begin{itemize}
        \item Google Analytics\cite{google_analytics}
    \end{itemize}
    &
    ...
    \hline
    \multirow{3}{*}{Citability}
    & \raggedright{Providing something to cite (Software DOI or manuscript) and information on how to cite} & 
    \begin{itemize}
        \item To create DOIs for software and more: Zenodo \cite{zenodo}, Dryad \cite{datadryad}, and Figshare \cite{figshare} 
        \item To track DOIs: Altmetric \cite{noauthor_altmetric_2015}
        \item To track paper citations: Semantic Scholar \cite{noauthor_semantic_nodate}
    \end{itemize} &
     \begin{itemize}
    ...
    \shortstack{Feedback \\Mechanisms} & 
    \begin{itemize}
    ...
    \end{itemize} & how often recipients open the news letter, click on links, or unsubscribe & \\
    \hline
    \multirow{3}{*}{Usability Testing}
    ...
    \hline
    \multirow{3}{*}{Socail Media}
    & \begin{itemize}
        \item YouTube Videos
        \item twitter
        \item instagram
        \item linkedin 
    \end{itemize} & &  Pairing engagement metrics (video watch counts, likes, etc) with software engagement metrics can determine if outreach strategies are successful & \\
    \hline
       \multirow{3}{*}{Reviews}
    & Review Forum & SourceForge &  Stars and review metrics can be directly used to reasure users and funders & \\ 
    \hline
  \end{tabular}
  \label{tab:inf_table}
\end{table}
...
\begin{table}
 \caption{\textbf{Software health infrastructure enables collecting metrics that can reassure users and funders.} More fine-grained tracking of how users are interacting with web-based tools or websites and can be helpful for later assessments, in addition to improving access, as one can also perform search engine optimization (SEO) to further encourage the visibility. Tools like Altmetric can indicate where  digital object identifiers(DOIs) are cited in multiple sources besides scientific articles, such as blogs, news articles, Wikipedia, social media, and more. }
  \centering
  \begin{tabular} {|p{0.1\textwidth}|p{0.15\textwidth}|p{0.15\textwidth}|p{0.23\textwidth}|p{0.24\textwidth}|}
    \hline
    \multicolumn{1}{|c|}{\cellcolor[gray]{.9} \textbf{Infrastructure}} 
    ...
    & \multicolumn{1}{|c|}{\cellcolor[gray]{.9} \textbf{Considerations}}\\[1.1ex]
    \hline
    \multirow{17}{*}{Version Control}               
    &   \multirow{3}{*}[-3em]{\shortstack{Continuous \\ Integration}} & \begin{itemize}
    ...
    \end{itemize} &  May need to consider privacy restrictions for tracking IP addresses\\
    \cline{2-5}
    & \multirow{5}{*}{ \shortstack{Discontinuous \\ Integration}} &
    \begin{itemize}
        \item Google Analytics\cite{google_analytics}
    \end{itemize}
    &
    ...
    Pages with more views may identify widely used features or confusing aspects \\
    \hline
    \multirow{3}{*}{Development Activity}
    & \raggedright{Providing something to cite (Software DOI or manuscript) and information on how to cite} & 
    \begin{itemize}
        \item To create DOIs for software and more: Zenodo \cite{zenodo}, Dryad \cite{datadryad}, and Figshare \cite{figshare} 
        \item To track DOIs: Altmetric \cite{noauthor_altmetric_2015}
...
\begin{table}
 ...
 to others.} Tool optimization can involve improving workflows, performance, usage, or usability. Evaluations can guide future work. For example, recognizing the types or volumes of data being used, as well as temporal trends of data usage, can highlight opportunities for new algorithm development. Similar analyses can help optimize resource allocation within and between projects. Evaluation can also aid in demonstrate tool value to gain external support, such as funding.  Evidence for impact is also often required to gain resources to develop or maintain semi- or un-related tools. Demonstration of past capability to develop impactful software supports requests to do so in the future.  Demonstrating that a tool is widely-used and widely-accepted can also encourage users to adopt a tool more readily, and be more invested in a tool community. Developers may be drawn to projects with impact to build upon an exiting tool. Finally, demonstration of impact can recruit new users which can diversify tool communities and bring new problems of interest that expand the utility of a tool.}
  ...
  %{|c|c|l|}
    ...
    & & 
    Identify strategies to reduce barriers \\
    \hline
